 ...
 * The Reshaping Engine It's role is to provide the right form of the word.
 * 
...
public class ArabicReshaper {
	 ...
	 * 
	 * @uml.property name="_returnString"
	 */
	...
	/**
	 * The All Lam Forms in the [0] element With all its forms either at the end
	 * of the word, or in the middle of the word in [1][2] respectively
	 */
	...
	/**
	 * All Arabic Letters with It's different Forms [0] Element is the letter
	 * Alone [1][2][3][4] are all the forms of the Letter [5] is the number of
	 * Forms the Letter has
	 * 
	 * If "ا" It has only two forms : "ـا" and "ا" If "ب" It has only four
	 * forms : "بـ", "ـبـ", "ـب" and "ب" and so on
	 */

	// The Array is sorted to be able to use the binary search, please don't
	// miss with it.
	...
	/**
	 * Constructor of the Class It doesn't support Alef Lam by Default
	 * 
	 * @param unshapedWord
	 *            The unShaped Word
	 */
	 ...
	 * @param unshapedWord
	 *            The unShaped Word
	 * @param supportAlefLam
	 *            To check If to support AlefLam or Not
	 */
	...
	/**
	 * Binary search for the character, it returns the index of the character in
	 * the array.
	 * 
	 * @param start
	 *            , the start point for the binary search
	 * @param end
	 *            , the end point for the binary search
	 * @param target
	 *            , the character that is function looking for
	...
	/**
	 * The function should return the correct Glphy type for the caller, the
	 * function Escape any character like "7arakat"
	 * 
	 * @param target
	 *            The character, that needed
	 * @param location
	 *            The location of the character to be return its type
	 * @return the integer number indicated the Number of forms the Character
	 *         has, return 2 otherwise
	 */
	...
	public String reshapeIt(String unshapedWord) {
		...
		for (int i = 1; i < wordLength - 1; i++) {
			...
			if (getGlphyTypeBefore(unshapedWord, beforeLast) == 2) { // checking
																		// if
																		// it's
																		// only
																		// has 2
		...
		if (getGlphyTypeBefore(unshapedWord, wordLength - 2) == 2) {
			...
			// position it is, It'll be always the second form
			reshapedWord
					.append(getReshapedGlphy(wordLetters[wordLength - 1], 1));
		} else {
			// Put the right form of the character, 4 for the last letter in the
			// word
			reshapedWord
					.append(getReshapedGlphy(wordLetters[wordLength - 1], 4));
		}
	...
	public String reshapeItWithLamAlef(String unshapedWord) {
		...
		// Check if the Word length is 1, then return the Reshaped One letter,
		// which is the same character of input
		if (wordLength == 1) {
		...
		if (wordLength == 2) {
			...
			if (getLamAlef(alef, lam, true) > 0) {
				return getLamAlef(alef, lam, true) + " ";
			}
		...
		// Iterate over the word from the second character till the second to
		// the last
		for (int i = 1; i < wordLength - 1; i++) {

			// Check if the Letters are Lam Alef
			if (getLamAlef(wordLetters[i], currentLetter, true) > 0) {
				// Check if the Letter before the Lam is 2 Forms Letter, to Make
				// the Lam Alef as its the end of the Word
				if (getGlphyTypeBefore(unshapedWord, i - 2) == 2) {
					...
					reshapedLetters[i] = getLamAlef(wordLetters[i],
							currentLetter, true);

				} else { // The Letter before the Lam is more than 2 Forms
							// Letter
				...
				// Check if the Letter Before Last has only 2 Forms, for the
				// current Letter to be as a start for a new Word!
				if (getGlphyTypeBefore(unshapedWord, beforeLast) == 2) {

					// If the letter has only 2 shapes, then it doesnt matter
					// which position it is, It'll be always the second form
					reshapedLetters[i] = getReshapedGlphy(wordLetters[i], 2);
				} else {

					// Then it should be in the middle which should be placed in
					// its right form [3]
		...
		// Check if the Letters are Lam Alef
		if (getLamAlef(wordLetters[wordLength - 1],
				wordLetters[wordLength - 2], true) > 0) {

			// Check if the Letter before the Lam is 2 Forms Letter, to Make the
			// Lam Alef as its the end of the Word
			if (getGlphyType(unshapedWord, wordLength - 3) == 2) { // check for
																	// the last
																	// letter
				...
				// Assign Lam Alef to the Letter of Alef
				reshapedLetters[wordLength - 1] = getLamAlef(
						wordLetters[wordLength - 1],
			...
			// check for the last letter Before last has 2 forms, that means
			// that the last Letter will be alone.
			if (getGlphyType(unshapedWord, wordLength - 2) == 2) {
				// If the letter has only 2 shapes, then it doesn't matter which
				// position it is, It'll be always the second form
				reshapedLetters[wordLength - 1] = getReshapedGlphy(
